  ...
  <section class="chain-block">
    ...
    <div class="perf-panel">
      ...
      <div class="perf-grid">
        ...
        <label class="perf-check" title="保持空闲连接更久，减少频繁重新建连；建议开启。" style="display:none"><input id="perfKeepAlive" type="checkbox" checked onchange="applyPerformanceSettings()">长连接</label>
...
<!-- Fake-IP removed: always redir-host -->
        ...
        <label class="perf-check" title="入口仅监听 127.0.0.1，外部无法直连本端口。"><input id="chainLocalhostOnly" type="checkbox" onchange="toggleChainLocalhostOnly()">仅允许本机访问</label>
...
function writeLog(text, append=false){
  ...
  el.innerHTML = lines.map((l, i) =>
    `<span style="opacity:${LOG_OPACITIES[i] || 0.1}">${escapeHtml(l)}</span>`
  ).join('\n');
  el.setAttribute('data-lines', JSON.stringify(lines));
  requestAnimationFrame(() => { el.scrollTop = 0; });
}
function out(msg){ writeLog(typeof msg === 'string' ? msg : JSON.stringify(msg,null,2), true); }
function appendLog(msg){ writeLog(typeof msg === 'string' ? msg : JSON.stringify(msg,null,2), true); }
function clearLog(){ const el=document.getElementById('output'); if(el){ el.innerHTML='等待操作。'; el.removeAttribute('data-lines'); } }
...
function renderChainSummary(){
  ...
  const loEl = document.getElementById('chainLocalhostOnly');
  if(loEl) loEl.checked = !!(state.chain && state.chain.localhost_only);
}
...
async function applyPerformanceSettings(){
  ...
  try{
    const settings = collectPerformanceSettings();
    const data = await api('/api/performance', {settings: settings});
    if(data.performance) renderPerformanceSettings(data.performance);
    if(data.ok){
      const isReload = data.stage === 'reload';
      out(`✓ ${isReload ? '已重启生效' : '已热加载生效'}`);
    } else if(data.validation && !data.validation.ok){
      out(`✗ 无法启动: ${(data.validation.output || data.error || '未知错误').split('\\n').slice(-3).join(' / ')}`);
    } else {
      out(`✗ 无法启动: ${data.error || '未知错误'}`);
    }
  } catch(e){
    out(`✗ 应用异常: ${e.message || e}`);
    await loadState(false);
  } finally {
    setPerformanceDisabled(false);
  }
}
async function setGlobalDns(choice){
  out(`正在设置所有服务端 DNS 为：${choice === 'local' ? '本地' : choice}`);
  (state.local_services || []).forEach(svc => {
    svc.dns_upstream = choice;
  });
  renderLocalCards();
  out(`已设置 ${state.local_services.length} 个服务端的 DNS 上游，正在应用到后台...`);
  try {
    ...
    if(data.ok) {
      out(`✓ DNS 配置已生效`);
      applyReturnedState(data);
    } else {
      out(`✗ DNS 应用失败: ${data.error || '未知错误'}`);
    }
  } catch(e) {
    out(`✗ DNS 应用异常: ${e.message || e}`);
  }
}
...
async function toggleChainLocalhostOnly(){
  ...
  // 同步更新 entry_text 中的 server 字段
  const ta = document.getElementById('entryText');
  if(ta && state.chain.entry_text){
    const newAddr = localhostOnly ? '127.0.0.1' : '0.0.0.0';
    state.chain.entry_text = state.chain.entry_text.replace(/^(\s*server:\s*)\S+/m, '$1' + newAddr);
    ta.value = state.chain.entry_text;
  }
  const chainEnabled = state.chain.enabled !== false;
  if(!chainEnabled){
    // 链式代理未启用，仅持久化设置
    await saveUiState();
    out(`✓ 已${localhostOnly ? '设为' : '取消'}仅允许本机访问`);
    out(`  ⚠ 链式代理未启用，启用后${localhostOnly ? '入口绑定 127.0.0.1' : '入口绑定 0.0.0.0'}`);
    return;
  }
  out(`正在应用仅允许本机访问...`);
  try {
    ...
    if(data.ok) {
      const isReload = data.stage === 'reload';
      out(`✓ 仅允许本机访问${localhostOnly ? '已生效' : '已关闭'} — ${isReload ? '已重启生效' : '已热加载生效'}`);
      applyReturnedState(data);
    } else if(data.validation && !data.validation.ok){
      out(`✗ 无法启动: ${(data.validation.output || data.error || '未知错误').split('\\n').slice(-3).join(' / ')}`);
    } else {
...
async function addChainNode(text=''){
  state.chain.node_texts = state.chain.node_texts || [['']];
  let template = text || '';
  if(!template){
    // 从上一级的第一个节点复制模板
    const prevLevel = state.chain.node_texts[state.chain.node_texts.length - 1];
    const prevFirst = (prevLevel && prevLevel[0]) ? prevLevel[0].trim() : '';
    if(prevFirst){
      const levelNum = state.chain.node_texts.length + 1;
      template = prevFirst.replace(/^(\s*-?\s*name:\s*)\S+/m, `$1chain-level${levelNum}-node1`);
    } else {
      template = `- name: chain-level${state.chain.node_texts.length + 1}-node1
  type: ss
  server: 127.0.0.1
  port: 18000
  cipher: aes-128-gcm
  ...
  state.chain.node_texts.push([template]);
  renderChainNodes();
  await applyAfterChainEdit('已增加下一级');
}
...
async function collectFreshStateForSave(){
  ...
  latest.chain.localhost_only = !!(state.chain && state.chain.localhost_only);
...
function syncGlobalDnsRadio(){
  const choices = new Set();
  (state.local_services || []).forEach(svc => {
    choices.add(svc.dns_upstream || 'local');
  });
  if(choices.size === 1){
    const single = [...choices][0];
    document.querySelectorAll('input[name="globalDns"]').forEach(radio => {
...
async function addBackupNode(levelIndex){
  if(!state.chain.node_texts[levelIndex]) state.chain.node_texts[levelIndex] = [''];
  ...
  if(firstNode.trim()){
    const nameMatch = firstNode.match(/^(\s*-?\s*name:\s*)(\S+)/m);
    if(nameMatch){
      const baseName = nameMatch[2].replace(/-(main|backup\d*)$/i, '');
      const backupCount = state.chain.node_texts[levelIndex].length;
      const newName = `${baseName}-backup${backupCount}`;
      template = firstNode.replace(/^(\s*-?\s*name:\s*)\S+/m, `$1${newName}`);
    } else {
      // 无 name 字段，直接复制并添加 name
      template = `- name: chain-node-backup${state.chain.node_texts[levelIndex].length}\n` + firstNode;
    }
  } else {
  ...
  renderChainNodes();
  await applyAfterChainEdit('已添加备用节点');
}
async function removeBackupNode(levelIndex, nodeIndex){
  if(!(state.chain.node_texts[levelIndex] && state.chain.node_texts[levelIndex].length > 1)) return;
  state.chain.node_texts[levelIndex].splice(nodeIndex, 1);
  renderChainNodes();
  await applyAfterChainEdit('已删除备用节点');
}
async function removeChainLevel(levelIndex){
  state.chain.node_texts.splice(levelIndex, 1);
  if(state.chain.node_texts.length === 0) state.chain.node_texts = [['']];
  renderChainNodes();
  await applyAfterChainEdit('已删除本级');
}
async function applyAfterChainEdit(label){
  const chainEnabled = state.chain.enabled !== false;
  if(!chainEnabled){
    await saveUiState();
    out(`✓ ${label}（已保存，启用链式代理后生效）`);
    return;
  }
  out(`正在应用 ${label}...`);
  try {
    ...
    if(data.ok){
      const isReload = data.stage === 'reload';
      out(`✓ ${label} — ${isReload ? '已重启生效' : '已热加载生效'}`);
      applyReturnedState(data);
    } else if(data.validation && !data.validation.ok){
      out(`✗ 无法启动: ${(data.validation.output || data.error || '未知错误').split('\\n').slice(-3).join(' / ')}`);
    } else {
...
function collectChainUiState(){
  ...
  const chainState = {
    ...
    localhost_only: !!document.getElementById('chainLocalhostOnly')?.checked,
    node_texts: []
  };
...
def build_managed_objects(state, used_listener_names=None, split_route=True):
    ...
    if chain.get("enabled") and chain.get("entry") and chain.get("nodes"):
        ...
        # 仅允许本机访问: 入口 listener 绑定 127.0.0.1
        if chain.get("localhost_only"):
...
def apply_state(state, skip_connectivity=False):
    if not state or not isinstance(state, dict) or not state.get("version"):
        state = load_yaml(STATE_PATH, default_state())
    with LOCK:
        ...
        reload = recreate_mihomo()
        if reload["ok"] and (next_state.get("chain") or {}).get("enabled") and not skip_connectivity:
...
def make_auth_file():
    ...
    if AUTH_PATH.exists():
        try:
            ...
            pass
        # Invalid or corrupted, delete and regenerate
...
def default_state():
    template = CONSOLE_DIR / "default-state.yaml"
    if template.exists():
        return load_yaml(template, {})
    return {"version": 1, "local_services": [], "chain": {"enabled": False, "entry_text": "", "node_texts": [[""]], "entry": None, "nodes": [], "exit_proxy": "", "localhost_only": False}, "managed": {"listener_names": [], "proxy_names": [], "proxy_group_names": []}, "connectivity": None, "performance": normalize_performance_settings()}
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/validate":
                ...
                with LOCK:
                    result, pending, _ = validate_state(s)
                    pending.unlink(missing_ok=True)
